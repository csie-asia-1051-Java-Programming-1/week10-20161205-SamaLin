...
//		Boolean flag=true;
		...
		for(int i=0;i<10;i++){
			try{
				ram=Integer.toString(scn.nextInt());
				if(Integer.valueOf(ram)<10){
					ISBN[0][i]=Integer.valueOf(ram);
				}
			}
			catch(Exception e){
				ram=scn.next();
				if(ram.equals("X")||ram.equals("x")){
					ISBN[0][i]=10;
				}else{
...
//		String BMI[]=scn.nextLine().split(" ");
//		if(BMI.length>10){flag=false;System.out.println("請輸入正確的ISBN");}
//		ArrayList<Integer>dataList=new Arraylist<Integer>();
	}
//	public int ISBNjdm(int ISBN[]){
//		int sum=0;
